...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options["all"]:
            ...
            self.stdout.write(
                self.style.WARNING("Начало полной синхронизации всех мероприятий...")
            )
        elif options["date"]:
            try:
                ...
                self.stdout.write(
                    self.style.WARNING(f"Синхронизация мероприятий за {sync_date}")
                )
            except ValueError:
                self.stderr.write(
            ...
            api_url += f"?changed_at={sync_date:%Y-%m-%d}"
            self.stdout.write(
                self.style.WARNING(f"Синхронизация вчерашних мероприятий ({sync_date})")
            )
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Синхронизация завершена: {stats['new']} новых, {stats['updated']} обновлено"
                )
            )
        ...
        for event in events_data:
            try:
                ...
                if created:
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"Создано новое мероприятие: {obj.name} (ID: {obj.id})"
                        )
                    )
                else:
                    self.stdout.write(
                        self.style.WARNING(
                            f"Обновлено мероприятие: {obj.name} (ID: {obj.id})"
                        )
                    )
                ...
                self.stderr.write(
                    self.style.ERROR(
                        f"Ошибка при обработке события {event.get('id')}: {e}"
                    )
                )
