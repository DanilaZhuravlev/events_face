    ...
    try:
        ...
        return {
            ...
            "registration_data": json.dumps(
                RegistrationResponseSerializer(registration).data
            ),
...
def async_confirm_registration(registration_id):
    try:
        ...
        is_expired = (
            registration.event.registration_deadline
        ...
        notification_type = (
            "registration_rejected" if is_expired else "registration_confirmed"
        )
        ...
        logger.info(
            f"Регистрация {registration_id} обработана. Статус: {registration.status}"
        )
